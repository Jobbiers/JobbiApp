...
const renameColor = (obj: { [key: string]: any }, prefix: string): { [key: string]: any } => {
  const renamedObj: { [key: string]: any } = {};
  for (const key in obj) {
    if (obj.hasOwnProperty(key)) {
      const newKey = `${prefix}${key.slice(4)}`;
...
const primary = renameColor(purple, 'primary');
const primaryDark = renameColor(purpleDark, 'primary');
